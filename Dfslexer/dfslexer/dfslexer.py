...
__all__ = ['DFSLexer', 'KalLexer', 'LiveScriptLexer', 'DartLexer',
...
class DFSLexer(RegexLexer):
    ...
    tokens = {
        ...
        'root': [
            ...
            (r'[0-9]+(ms|s|m|d|h|w)', String.Doc),
            (r'^(?=\s|/|<!--)', Text, 'slashstartsregex'),
            (r'(<<<).*(>>>)$', String),
            include('commentsandwhitespace'),
            (r'(\.\d+|[0-9]+\.[0-9]*)([eE][-+]?[0-9]+)?', Number.Float),
            # (r'\b(\.[$a-zA-Z_][\w.$]*)?\b', Name.Function),
            ...
            (r'AND|OR|and|or|andalso|orelse', Operator.Word),
            ...
            (r'(change_detect|combine|crate_query|deadman|debug|default|delete|email|http_post|'
             r'http_post_crate|join|json_emitter|keep|log|mem|modbus|mqtt_publish|mqtt_subscribe|'
             r'oracle_query|python|rename|s7poll|sample|set|shift|state_count|state_duration|'
             r'state_sequence|stats|tcp_receive|tcp_receive_line|time_diff|time_elapsed|'
             r'triggered_timeout|union|value_emitter|where|win_clock|win_event|win_time|'
             r'case|eval|rate|amqp_consume|amqp_publish|batch)\b', Name.Builtin),
            (JS_IDENT, Name.Other),
            (r'"(\\\\|\\"|[^"])*"', String.Double),
            (r"'(\\\\|\\'|[^'])*'", String.Single),
            (r'`', String.Backtick, 'interp'),
            # (r'<<<', String.Double, 'interp')
        ],
        'interp': [
            # (r'(<<<)', String.Double, 'interp-text-inside'),
...
class KalLexer(RegexLexer):
    ...
    tokens = {
        ...
        'root': [
            ...
            (r'\?|:|_(?=\n)|==?|!=|-(?!>)|[<>+*/-]=?',
             Operator),
            (r'\b(and|or|orelse|andalso|isnt|is|not|but|bitwise|mod|\^|xor|exists|'
