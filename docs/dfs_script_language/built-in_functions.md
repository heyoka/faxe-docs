...
| `env(Key::string)` -> string \| false                       | Get the value from an ENV key, if not found, returns `false`. Note that the value returned (except false) is always as string.     | env('FAXE_MQTT_HOST') -> '127.0.0.1'             |
| `env(Key::string, Default)` -> string \| any                | Get the value from an ENV key, if not found, returns `Default`. Note that the value returned (except Default) is always as string. | env('FAXE_MQTT_HOS', '127.0.0.1') -> '127.0.0.1' |
