...
| queue_type( `string` )               | Queue type, Valid values are: `''`, `'quorum'`, `'classic'`. The value will be used for the "x-queue-type" argument while declaring a queue. With `''`, the type of the queue will be the default type on the vhost. | ''                                      |
...
| takeover_queue_type ( `string` )     | Queue type for the take-over queue. Valid values are: `''`, `'quorum'`, `'classic'`. The value will be used for the "x-queue-type" argument while declaring a queue. With '', the type of the queue will be the default type on the vhost.                                                | ''                                          |
